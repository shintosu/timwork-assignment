...
- 오버레이 비교(최소 기능 구현)
  - 위치: `src/App.tsx` (Stage 컴포넌트)
  - 기능: 베이스/오버레이 2중 선택 + 투명도 슬라이더. 오버레이는 `imageTransform`(x,y,scale,rotation)을 CSS transform(translate/rotate/scale)으로 적용하여 기준 이미지 위에 정렬
  - 기준 이미지 선택: 오버레이를 켠 경우 `imageTransform.relativeTo`를 기준으로 사용, 꺼진 경우 선택 레이어의 이미지 사용
  - 스케일링: 기준 이미지의 naturalWidth/Height를 측정해 컨테이너에 맞춰 균일 스케일 적용(`transform: scale(...)`, origin top-left)
...
- 겹쳐보기: 베이스/오버레이 2중 선택 + 투명도 슬라이더를 제공. 오버레이는 `imageTransform` 기반으로 CSS transform(translate/rotate/scale)으로 정렬. 향후 `referenceGroups`로 같은 기준(relativeTo)만 후보로 제한하여 선택 실수를 줄일 계획.
  
  현재 구현: 베이스/오버레이 2중 선택 및 투명도 슬라이더가 활성화되어 있으며, 오버레이 정렬은 CSS transform으로 수행한다. 후보 제한/추천(`referenceGroups`)은 차후 도입 예정.
...
이러한 최소 뷰는 사용자 시나리오의 첫 관문(“빠르게 찾아보고 확인”)에 집중합니다. 현재 베이스/오버레이 2중 선택과 투명도 슬라이더까지 제공하여 “간섭 확인”을 1차 지원합니다. 이후 리비전 타임라인과 `referenceGroups` 기반 후보 제한을 추가하면 “변경 이력 추적”과 오버레이 선택 정확도를 높일 수 있습니다.
